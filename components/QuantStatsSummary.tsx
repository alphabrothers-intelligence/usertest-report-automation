...
const sectionTitle = "mt-6 text-base font-semibold text-zinc-900 dark:text-zinc-50 first:mt-0";
...
export function QuantStatsSummary({ s }: { s: QuantStats }) {
  ...
  return (
    <div className="w-full rounded-lg border border-zinc-200 bg-white px-5 py-4 text-base text-zinc-800 dark:border-zinc-800 dark:bg-zinc-900 dark:text-zinc-100">
      ...
      <p className={sectionTitle}>Ⅱ. 인적사항</p>
      ...
      <p className={sectionTitle}>Ⅲ. 기능별 만족도</p>
      <MeanSdTable rows={s.featureSatisfaction} />

      <p className={sectionTitle}>Ⅳ. 핵심구매요소</p>
      <p className={`mt-2 ${subLabel}`}>상대중요도 (높은 순)</p>
      ...
      <p className={sectionTitle}>Ⅴ. 4대가치 만족도</p>
      <MeanSdTable
        rows={[
      ...
      <p className={sectionTitle}>Ⅵ. UX 품질</p>
      <p className={`mt-2 ${subLabel}`}>실용성</p>
      <MeanSdTable rows={s.uxQuality.usability} />
      <p className={`mt-3 ${subLabel}`}>즐거움</p>
      <MeanSdTable rows={s.uxQuality.fun} />

      <p className={sectionTitle}>Ⅷ. 종합만족도 · NPS</p>
      <ul className="mt-2 space-y-1 text-sm">
      ...
      <p className={sectionTitle}>Ⅶ. 교차분석</p>
      <p className={`mt-2 ${subLabel}`}>연령대별 (기능별 만족도 평균)</p>
      <div className={tableWrap}>
        <table className={table}>
          <thead>
            <tr>
