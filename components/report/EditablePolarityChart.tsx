        ...
        <svg ref={svgRef} data-export-crop-top="32" viewBox={`0 0 ${width} ${height}`} className="block w-full" aria-label={`${block.title} 긍정, 부정, 중립 응답 비율`}>
          ...
          {values.map((item) => {
            ...
            return <g key={item.key}>
              <path d={halfDonutPath(cx, cy, outerRadius, innerRadius, startDegree, endDegree)} fill={colors[item.key]} stroke="#ffffff" strokeWidth="2" />
          ...
          {values.map((item, index) => <g key={`${item.key}-legend`}>
            <rect x="475" y={108 + index * 28} width="14" height="14" rx="1" fill={colors[item.key]} />
