...
원인 파악에 시간이 걸린 에러·장애는 이 파일에 바로 적지 말고 [docs/TROUBLESHOOTING.md](./docs/TROUBLESHOOTING.md)에
증상→원인→해결 형식으로 기록한다(새 항목은 파일 위에 추가). 낯선 에러를 조사할 때는 그 문서를
먼저 확인할 것 — 이미 겪은 문제일 수 있다.
...
- `check:qualitative-fidelity`가 PRD 9.2절 **극성 판정 일치율**을 잰다. **2026-09-02 전면 교체** —
  예전 판은 손으로 옮겨 적은 24건을 `runStage1`(지금 제품이 안 쓰는 절 분리 경로)에 넣어 쟀다.
  제품과 무관한 숫자였고 부정/중립 표본이 14건뿐이라 1건 차이로 합격선을 넘나들었다.
  - **정답은 발행 보고서 PDF에서 `pdftotext`로 뽑는다**(8~26쪽, 94건). 손으로 옮겨 적지 않으므로
    표본을 늘리는 데 비용이 안 든다. **예측은 DB에 저장된 그 보고서의 카테고리**(제품 경로가
    실제로 만든 결과)에서 같은 인용문이 어느 극성에 들어갔는지로 본다 — **API 호출 0, 과금 0**.
  - 두 가지를 같이 낸다. ① **대표 인용 일치율**(제품이 뽑아 실은 인용문에 한정) ② **문항별 극성
    비율**(원본이 배너에 찍어둔 %와 대조, 응답 전수라 편향 없음). ①만 보면 "제품이 자신 있게
    뽑은 문장"만 재는 셈이라 과대평가된다 — 미매칭 건수와 ②를 반드시 같이 볼 것.
  - **기준선(2026-09-02, 리바랩스 report fa315b97)**: 정답 94건 중 39건 매칭(직접 20 + 같은 응답
    19). 긍정 vs 나머지 **78.9%**(15/19, 목표 90%), 부정 vs 중립 **75.0%**(15/20, 목표 75%),
    전체 76.9%. 문항별 비율 **평균 절대 오차 8.0%p**(18개 비교).
  - **두 지표가 같은 곳을 가리킨다 — 중립 과다 배정.** 펫 레이싱 중립 3.5→28.7%, 펫 성장
    3.0→20.0%, 펫과의 산책 3.0→14.1%로 원본보다 훨씬 많고 그만큼 부정이 줄었다(펫 레이싱 부정
    68.0→47.5%). ③(프롬프트 수정)은 이 기준선 위에서 **부정↔중립 경계 정의**를 손대고 전후를
    비교하는 것이 순서다. `FAST_STANDARD_SYSTEM`이 대상이며 PRD 6장 프롬프트 불변 원칙과 무관하다.
  - **③ 프롬프트 수정 결과(2026-09-04)**: `FAST_STANDARD_SYSTEM`의 극성 규칙을 고쳤다. 예전엔
    "단순 개선 제안·선호·정보 요청은 **구체적 손해가 없으면 neutral**" 한 줄뿐이라 불만족
    표현이 통째로 중립으로 샜다. 지금은 **불만·아쉬움·실망을 드러내면 구체적 손해가 없어도
    negative**, neutral은 **좋고 나쁨을 말하지 않은 응답에만**(사실 서술·취향/해당 없음·불만
    없는 제안)으로 좁혔다. `npm run check:fast-polarity`(신규)로 같은 스크립트·같은 6문항을
    옛 프롬프트/새 프롬프트로 실제 실행해 대조한 결과:

    | | 옛 프롬프트 | 새 프롬프트 1회 | 새 프롬프트 2회 |
    |---|---|---|---|
    | 부정 vs 중립 | 57.1%(8/14) | **84.6%(11/13)** | **91.7%(11/12)** |
    | 긍정 vs 나머지 | 100%(16/16) | 100%(19/19) | 100%(17/17) |
    | 비율 평균 오차 | 6.7%p | 8.0%p | 6.7%p |

    중립 비율도 원본에 가까워졌다(펫과의 산책 14.5→7.8%, 원본 3.0 / 펫 레이싱 21.4→14.0%,
    원본 3.5). **비율 오차는 개선되지 않았다** — 비율은 모델이 스스로 세는 `clause_count`
    합에서 나오므로 극성 정확도와는 다른 축이다. 비율을 더 맞추려면 세는 방식(모델 추정 →
    응답 수 기준 등)을 손대야 하고, 그건 별도 작업이다.
  - **개수는 코드가 센다(2026-09-04)**: 예전엔 모델이 카테고리마다 `clause_count` 숫자를
    적어서 냈다 — 언어 모델이 가장 못 하는 일(개수 세기)을 시킨 것이라 총합과 카테고리 합이
    어긋나 **문항이 통째로 실패**하고(실측) 실행마다 비율이 흔들렸다. 지금은 앵커 경로에서
    **`respondents`(해당 응답자 번호 목록)를 받아 코드가 센다**(`anchorQuotes.ts`). 입력에 없는
    번호와 중복은 버리고(`stats.droppedRespondents`), `total_clause_count`도 코드가 합산하므로
    합계 불일치가 구조적으로 불가능하다. **"판단은 모델, 계산은 코드"** — 새 집계값을 만들 때
    이 원칙을 따를 것. `npm run check:anchor-quotes`(18건)가 고정한다.
    - 목록은 `categories.respondents`(int[])에 저장한다 — 극성 채점을 대표 인용 몇 건이 아니라
      전수로 할 수 있게 하려는 것이다(아직 그 채점 경로는 안 만들었다).
  - **재실행 결과(2026-09-04, 새 프롬프트 + 새 집계로 리바랩스 report 재생성)**: 14문항 전부
    성공, 6.9분(4워커). 저장된 결과 기준 채점이 전부 좋아졌다 — 긍정 vs 나머지 78.9→**100%**,
    부정 vs 중립 75.0→**78.9%**, 전체 76.9→**86.2%**, 비율 평균 오차 8.0→**6.6%p**.
    **비율 오차는 크게 안 줄었다** — 집계를 결정론적으로 바꿔도 "어떤 응답이 어느 카테고리냐"는
    여전히 모델 판단이고, 원본은 한 응답을 여러 의미 단위로 쪼개 세기 때문이다(우리는 카테고리당
    응답자 1회). 더 줄이려면 세는 **단위**를 원본과 맞추는 별도 작업이 필요하다.
  - **한 번 돌린 결과로 판단하지 말 것.** 같은 프롬프트로 두 번 돌려도 비율 오차가 8.0 ↔ 6.7%p로
    움직인다(모델 비결정성). 프롬프트를 고칠 때는 최소 2회씩 돌려 방향이 유지되는지 볼 것.
  - 파일명으로 보고서를 고르지 말 것 — 업로드 경로에서 온 `file_name`은 한글이 자모 분리(NFD)로
    저장돼 `like '%리바랩스%'`(NFC)에 안 걸린다(2026-09-02 실측).
...
## 검토 표시를 왼쪽 `분석 근거` 패널 한 곳으로 (2026-09-02)

담당자가 확인해야 할 것(정량 요주의 표시 · 도표 계산 근거 · 정성 극성 확인)을 **전부 웹뷰
왼쪽 패널**에 모았다. 문서 본문에 안내 박스를 새로 넣지 않는다는 기존 원칙(`quantEvidence.ts`
머리말) 그대로이며, 승인 게이트가 아니라 표시다(memory: review-flag-not-gate).

- `ReviewFlagNotice`가 정량 `ReviewFlag`(`lib/quant/reviewFlags.ts`)를 그린다 — 공통 모양은
  `ReviewNotice`. 예전엔 문서 안 도표 위에 붙어 있었고, 지금은 `useReportEvidence`가 읽는
  위치(`readingBlockId`)에 맞춰 패널에서 바뀐다.
- **근거 항목은 라벨 한 줄 + 내용 아래로 쌓는다**(`AnalysisReferenceContent`). 굵은 라벨 옆에
  내용을 이어 붙이면 여섯 줄이 다 같은 무게로 보여 "그래서 내가 뭘 봐야 하나"가 안 보인다는
  지적(2026-09-02). `확인할 내용`·`검증할 부분` 두 줄만 "직접 확인" 박스로 강조한다 —
  나머지는 참고용 사실이다. 새 근거 항목을 추가할 때 이 두 갈래(사실 vs 사람이 할 일)를
  유지할 것.

### 극성 판정 확인 — 경고가 아니라 결정 카드

- **규칙은 카테고리 단위**(`categoryPolarityNeedsReview`, `lib/pipeline/confidence.ts`).
  보고서가 실제로 쓰는 fast 경로에는 절이 없어 `scoreConfidence`(절 단위)를 걸 자리가 없다.
  부정·중립 카테고리 중 **인용문에 감정어만 있고 구체적 손해 서술이 없는** 묶음만 표시한다.
  - **판정에 카테고리 라벨을 넣지 말 것** — Stage2 라벨은 "~부족", "~불편"처럼 감정어를 거의
    항상 담고 있어 라벨까지 보면 구체적인 묶음이 전부 딸려 온다(실측: 리바랩스 report의
    부정·중립 121개 중 34개가 걸렸고 대부분 오탐).
  - 낱말 목록만으로는 상세한 불만("연출이 부족하여 보는 재미가 심각하게 떨어졌다")을 못
    걸러서 인용문 길이(`SHORT_QUOTE_CHARS`) 조건을 같이 쓴다 — 같은 데이터에서 10건(8%)으로
    줄고 남은 것이 실제로 부정↔중립 경계인 묶음이었다.
- **표시 위치는 "지금 읽고 있는 그 묶음"이다.** `categoryHtml`이 카테고리마다
  `<div data-quote-category ...>` 래퍼(문서에 보이는 변화 없음)를 씌우고, 확인이 필요하면
  `data-polarity-review`를 붙인다. 패널은 읽는 지점의 카테고리 → 없으면 그 극성 그룹 순으로
  찾는다. **블록 전체에서 모으면 안 된다** — 긍정 의견을 읽는 중에 부정 묶음 카드가 떠 있는
  어긋난 화면이 됐다(2026-09-02 지적).
- **카드는 판단 재료와 결정 버튼을 함께 준다**(`PolarityReviewCard`): 묶음 이름 + 한 줄 사유 +
  응답 원문(판정을 흔들리게 한 표현에 형광펜) + `[현재 극성 유지]` `[다른 극성으로 변경]`.
  "확인이 필요합니다"만 띄우고 아무 행동도 못 하게 하면 도움이 안 된다는 지적에서 나온 형태다.
  **판단 기준은 별도 문단으로 쓰지 말 것** — 첫 판은 사유·기준·형광펜 설명·되돌리기 안내를 다
  문장으로 적어 18줄이 넘었고 "읽을 게 많아 빨리 판단할 수 없다"는 지적을 받았다(2026-09-02).
  지금은 기준을 **버튼 밑 한 마디**("구체적 불편 있음" / "취향·단순 감상")로 붙인다 — 기준을
  읽는 곳과 고르는 곳이 같아야 한 번에 끝난다. 검토용 카드를 새로 만들 때 이 형태를 따를 것.
- **결정은 DB에 반영하고 화면은 서버가 다시 만든 블록으로 교체한다**
  (`POST /api/report-workspace/category-polarity` → `reviewCategoryPolarity`). 극성이 바뀌면
  배너 번호·비율·도넛이 전부 달라지는데 그 계산은 이미 `buildReportWorkspaceSeed`에 있다 —
  **브라우저에서 DOM을 옮기고 비율을 다시 계산하지 말 것**(로직이 두 벌이 된다). 교체 범위는
  그 문항 블록(`...-q3-intro`/`-chart`/`-detail`)뿐이라 다른 편집 내용은 남는다.
  `categories.polarity_reviewed`(신규 컬럼, `npm run db:migrate` 필요)가 true면 표식을 더 안
  띄운다 — 게이트가 아니라 "이미 봤다" 표시다.
- `npm run check:polarity-review`가 규칙과 "보고서 HTML에 표식이 실리는지"를 같이 검사한다
  (API·DB 불필요). 라이브 검증은 실제 브라우저에서 카드 버튼 클릭 → DB 반영 → 문서 갱신 →
  표식 소멸까지 확인했다(2026-09-02, `npm run shot:web` 및 CDP 클릭).
- 이 표시는 **흔들리는 판정을 눈에 보이게 할 뿐 정확도를 올리지 않는다.** 부정↔중립 일치율
  자체를 개선하려면 먼저 측정 도구를 제품이 쓰는 앵커 경로로 옮기고 표본을 늘려야 하고
  (`check:qualitative-fidelity`는 지금 쓰지 않는 `runStage1` 경로를 잰다), 프롬프트
  (`FAST_STANDARD_SYSTEM`) 수정은 그 기준선이 생긴 뒤에야 전후 비교가 가능하다.

### 왼쪽 패널은 "읽을 글"이 아니라 "할 일"을 먼저 보여준다 (2026-09-02)

여섯 줄짜리 근거 목록을 다 읽어야 뭘 확인해야 하는지 알 수 있어 검토가 느리다는 지적으로,
`AnalysisReferenceContent`를 **기본은 "직접 확인할 것" 한 덩어리만** 보여주고 계산 과정·데이터
출처는 `<details>`로 접었다. 근거 항목을 새로 추가할 때 이 두 갈래(사람이 할 일 vs 참고용 사실)를
유지할 것 — `ACTION_BULLET_LABELS`에 있는 라벨만 펼친 채로 나온다.

### 인용문 끝맺음 — 해체(반말)·어간형도 검토 대상 (2026-09-02)

"…엉성하게 느껴져", "…진입장벽이 좀 높아", "…이유가 부족하여"처럼 반말·어간으로 끝난 인용문도
보고서에는 끝맺음이 안 된 문장이다. `HAECHE_ENDINGS`(져·워·해·돼·봐·줘·아·어·여·지·게·걸)로
표시하고, 한 음절만 바꾸면 격식체가 되는 것(져→집니다, 해→합니다, 아/어/여 어간 정리)만
`deterministicEndingCompletion`이 고친다 — 나머지(워·지·게·걸)는 표시만 하고 LLM 경로에 맡긴다.
실측 기준: 리바랩스 최신 report 인용 518건 중 9건이 새로 걸렸고 전부 실제로 끝이 잘린 문장,
명사 오탐(단어·이미지 등)은 없었다. `npm run check:quote-ending`(30건)이 회귀를 막는다.

### 정성 인용문 글자 크기 — 원본 실측값 (2026-09-02)

`pdftotext -bbox`로 원본 PDF에서 직접 잰 값이다(bbox 높이는 명목 크기의 약 1.11배 — 표 셀이
11pt인데 12.2pt로 찍히는 것으로 환산 비율을 확인). **눈대중으로 고치지 말고 이 값을 쓸 것.**

| 자리 | 원본(명목) | 우리 구현 |
|---|---|---|
| Ⅲ장 인용문(한 칸, 15·20쪽) | 10pt | 13px(=9.75pt) — RichReportEditor 본문 |
| Ⅲ장 카테고리 라벨 | 11pt | 10.5pt |
| Ⅲ장 극성 배너 | 13pt | 배너 스타일 |
| **Ⅴ장 인용문(두 칸, 34쪽)** | **9pt** | `VALUE_COLUMN.quote` 9pt |
| Ⅴ장 조사 결과 박스 | 10pt | 표 셀 11pt |

Ⅴ장이 Ⅲ장보다 작은 건 실수가 아니라 **두 칸으로 나뉜 자리라 원본이 원래 더 작다**. rich-static
블록은 편집기 클래스를 안 거치므로 `<td>`에 글자 크기를 명시하지 않으면 문서 기본값(16px)이
상속된다 — 상속을 믿지 말 것.

### 인용문 근거 강조(볼드+밑줄)가 통째로 비는 사고 (2026-09-02)

- **증상**: Ⅴ장 기능적·심미적·경제적 가치의 긍정/부정 인용문에 강조가 하나도 없었다. 실측하니
  14문항 중 6문항이 강조 0건(4대 가치 세 문항 + 펫 교배·펫 성장 시스템·전반적 만족도·유사 서비스).
- **원인**: `buildQuoteDisplayText`가 `evidence.quote === quote`(모델이 인용문을 토씨까지 똑같이
  옮겨 적었을 때)와 `quote.includes(reasonSpan)`(구간도 완전 일치)를 **둘 다** 요구했다. 모델이
  옮겨 적으며 띄어쓰기 하나만 달라져도 그 문항의 강조가 전부 사라졌다. 실패가 조용해서(로그도
  없음) 발행된 보고서를 사람이 보고서야 발견됐다.
- **해결**: 공백 무시 매칭(`looseRange`) + "이 인용문 안에서 찾아지는 구간" 폴백. **안전장치는
  그대로다** — 구간이 실제로 그 인용문 안에 연속으로 있어야 하고, 4자 미만이거나 문장의 90%를
  넘으면 강조하지 않는다(무분별한 볼드 금지). 같은 문항을 다시 돌려 3/3 강조가 나오는 것을
  확인했다. `warnIfNoEvidenceHighlight`가 강조 0건일 때 evidence 개수와 함께 경고를 남긴다 —
  "모델이 안 준 것"과 "줬는데 못 찾은 것"을 로그로 구분할 수 있다.
- **인용문 길이 상한 130자(2026-09-04)**. 설문 답변 한 칸에 "장점: 1. … / 2. … 단점: 1. …"처럼
  여러 논점이 들어 있는 경우, 모델이 그 전체를 한 인용문으로 지목해 부정 카테고리에 장점까지
  딸려 들어갔다(담당자 지적, 실측 326자). 원본 발행 보고서의 인용문은 **중앙값 48자·최대 129자**
  이므로 그 상한을 넘을 때만 `narrowQuoteToEvidence`가 **근거 구간이 들어 있는 조각 하나로
  좁힌다**(문장 끝·`/`·`1.` 경계로 자름, 경계 문자는 조각에서 뺀다). 좁히는 방식이 "원문의 연속
  구간 고르기"뿐이라 verbatim은 유지된다. 근거 구간이 없으면 좁힐 기준이 없어 그 인용문은 버린다.
  좁힐 때 지키는 것 셋(2026-09-07 보완):
  - **조건·이유 절은 데려온다.** 앞 조각이 `CONNECTIVE_ENDINGS`(`lib/report/quoteEnding.ts`,
    인용문 끝맺음 검사와 **같은 목록**)로 끝나면 함께 포함한다 — "겨울이라 잘 못 나가서 그런지 /
    보상이 적게 느껴졌어요"에서 뒤만 남기면 응답자가 붙인 단서가 사라진다.
  - **근거가 두 조각에 걸쳐 있으면 그 조각들을 묶는다.** 한 조각 안에 있을 때만 좁히던 판에서는
    통째로 포기해 372자짜리가 그대로 남았다(실측).
  - **버려서 그 카테고리의 인용이 0건이 될 때는 남긴다**(`stats.overlongKept`).
  - 경계 판정 함정: **소수점을 문장 끝으로 보면 안 된다** — "하루 1.5km"가 잘려 인용문이
    "5km 정도를…"로 시작했다(실측). 마침표류는 뒤에 공백/문장 끝이 올 때만, 번호 항목은 점 뒤가
    숫자가 아닐 때만 경계로 본다.
  실측: 문제의 문항을 다시 돌리니 최대 372자 → **114자·130자 초과 0건**, 근거 강조 96%.
  `check:fast-polarity`가 실행마다 최대 길이와 초과 건수를 출력한다.
- **근거 구간(`reason_from`/`reason_to`)을 선택 항목으로 두지 말 것(2026-09-04)**. optional이던
  동안 모델은 이 필드를 **문항 통째로** 건너뛰었고, 그 문항 인용문에는 강조가 하나도 안 붙었다
  (실측: 재생성한 보고서 14문항 중 7문항이 0건, 나머지는 27/27처럼 거의 전부 — 전부 아니면
  전무였다). 필수로 바꾼 뒤 같은 문항에서 60건 중 50건(83%)이 강조됐다. 필수로 둬도 위험은
  없다 — 위치가 틀리면 코드가 인용문 안에서 못 찾아 표시만 생략되고 인용문은 남는다.
  `check:fast-polarity`가 실행마다 강조 비율을 함께 출력하므로 다시 비면 바로 보인다.
- `npm run check:quote-evidence`(8건)가 회귀를 막는다. **이미 저장된 보고서는 고쳐지지 않는다**
  — quoteEvidence는 저장하지 않고 버리므로, 강조를 채우려면 그 문항의 정성 분석을 다시 돌려야 한다.

### 패널 폭 · 편집 아이콘 (2026-09-02)

- `분석 근거`·`보고서 작업` 패널은 52px 세로 탭으로 접힌다. **목차는 접으면 열 자체가 사라진다**
  (세로 탭도 안 남긴다 — 2026-09-02 담당자 요청 "넣으면 아예 들어가도록"). 그래서 다시 여는
  버튼은 위 툴바의 사이드바 아이콘(`SidebarIcon`)이고, 상태는 `onToolbarActionsChange`로
  ReportStudio에 올린다. 목차를 접으면 그 폭을 근거 패널이 가져간다(320→420px).
- 툴바의 되돌리기·다시실행·굵게·기울임·밑줄은 글자 라벨이 아니라 아이콘이다(`UndoIcon`,
  `FormatGlyph`). 뜻은 `title`/`aria-label`로 남긴다.

### 웹뷰 3열 레이아웃 — 본문 열은 A4보다 좁아지면 안 된다

- 본문 열 최소 폭은 **`minmax(210mm, 1fr)`**. `minmax(560px, 1fr)`이던 시절엔 화면이 좁으면
  트랙(560px)이 문서(794px)보다 좁아져서 **문서가 오른쪽 편집 패널을 뚫고 나갔다**(담당자가
  두 번 지적). 트랙이 A4보다 좁아질 수 없게 하는 것이 근본 해결이고, 대신 좁은 화면에서는
  페이지가 가로로 스크롤된다. 그래서 오른쪽 `보고서 작업` 패널도 왼쪽 패널처럼 접을 수 있게
  했다(52px 세로 탭) — 1440px에서 왼쪽·오른쪽 중 하나만 접으면 가로 스크롤 없이 들어간다.
- **스크롤해서 도표 앞에 도착하면 오른쪽 편집 탭이 클릭 없이 그 도표로 열린다.** 훅이
  `onReadingBlockChange` 콜백으로 알려준다(스크롤 rAF 안에서 실행 — effect 안에서 setState
  하면 `Calling setState synchronously within an effect` lint 에러가 난다). 본문 문단(`text`)
  에서는 선택을 바꾸지 않는다 — 글을 읽는 동안 방금 고르던 차트의 편집칸이 사라지면 방해된다.
