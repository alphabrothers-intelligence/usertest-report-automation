...
## 현재 구현 범위 (Phase 7까지 — v1 로드맵 핵심 기능 전체)
...
- 남은 것은 PRD 12장 Phase 8(회귀테스트·fidelity 측정 자동화 — `check:golden`이 이미 상당 부분
  커버하지만 정성 파이프라인 fidelity 자동 측정은 아직 없음)과 Phase 9(동시성·레이트리밋 튜닝,
  13장 실험 계획 실행)뿐이다. 둘 다 새 기능이 아니라 기존 파이프라인의 검증·튜닝 작업이다.
- 알려진 기능 갭: 제품 정보 입력 UI(5.0절, 위 참고), Tier 2/3 입력 경로(구조화된 요약본·자유서술
  요약, 5.2·5.3절 — v1은 Tier 1 raw data 경로만 구현), 자유형식 요약 추출용 체크포인트 0(7.0절).
