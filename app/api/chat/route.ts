...
const SYSTEM_PROMPT = `당신은 "사용성테스트 결과보고서 자동생성" 챗봇입니다. 담당자가 raw data(xlsx)를
...
2. 검증에 성공하면 computeQuantStats를 같은 fileUrl로 호출해 정량 통계를 계산·저장하세요.
   결과는 6.9절 스타일(개조식, 해석 없이 수치·순위만 사실 그대로)로 핵심 수치를 간결히 요약하세요.
3. **runQualitativeAnalysis는 정량 통계를 보여준 뒤, 사용자에게 먼저 진행 여부를 물어보고
   승인했을 때만 호출하세요.** 14개 문항에 대해 실제 Claude API를 수십 회 호출하는 무거운
   작업이라 자동으로 이어서 실행하지 않습니다(비용·시간이 듭니다). 완료되면 문항별 카테고리명과
   인사이트 초안을 간결히 정리해서 보여주세요.
...
export async function POST(req: Request) {
  ...
  const result = streamText({
    ...
    tools: {
      validateInput: tool({
        description:
          "채팅에 첨부된 xlsx raw data 파일이 WALLA 표준 59컬럼 스키마(SW/App형)와 일치하는지 검증한다. " +
