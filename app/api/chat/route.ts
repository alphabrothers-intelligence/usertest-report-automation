...
2. **validateInput이 성공하면 곧바로 presentProductInfoPrompt를 호출해 기업/제품 정보
   ...
   - **사용자가 건너뛰겠다고 하면 validateInput이나 presentProductInfoPrompt를 다시 호출하지
     말고, 저장도 하지 말고 곧바로 3번(presentReportPlan)으로 넘어가세요.**
...
4. **목차 계획에 동의를 받으면 computeQuantStats를 같은 fileUrl로 호출해 그래프용 응답 수치와 비율을
   계산·저장하세요.** 도구 결과는 화면에 이미 완전한 카드(인적사항·기능별 만족도·핵심구매요소·
   4대가치·UX품질·NPS·교차분석 전부 포함)로 렌더링되니 같은 수치를 채팅 텍스트로 다시
   요약·나열하지 마세요. 대신 **"방금 보여드린 목차·섹션에 들어갈 그래프용 응답 수치와 비율을 raw data로
   계산한 결과입니다" 같은 한 문장으로, 이 카드가 바로 위 목차 계획과 어떻게 연결되는지
...
# 원칙
- runQualitativeAnalysis가 만드는 insight, generateRecommendation이 만드는 제언 문장은 전부 AI 초안입니다. 보고서 화면에서 수정할 수 있다고 안내하세요.
  ...
  const tools = {
    ...
    assembleReportTool: tool({
      description:
        ...
        "각각의 다운로드 링크를 반환한다. 분석 초안은 보고서 화면에서 사용자가 수정할 수 있으므로 " +
        "미승인 항목이 있다는 이유로 발행을 막지 않는다.",
      inputSchema: z.object({
        fileUrl: z.string().url().describe("validateInput에 사용했던 것과 동일한 raw data URL"),
      }),
      execute: async ({ fileUrl }) => {
