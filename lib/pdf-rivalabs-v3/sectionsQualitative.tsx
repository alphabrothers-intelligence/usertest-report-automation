...
/** 원본의 표·도표 위 연보라색 제목 띠. 모든 종합표에서 같은 위계를 유지한다. */
function LavenderTitleBand({ title }: { title: string }) {
  return (
    <View
      style={{ backgroundColor: colors.chartBannerBg, paddingVertical: 4, marginTop: 8, marginBottom: 5 }}
      wrap={false}
    >
      <Text style={{ fontSize: 10, fontWeight: "bold", textAlign: "center", color: colors.text }}>{title}</Text>
    </View>
  );
}
...
export function SectionFeatureExperience({
  ...
  return (
    /* 장 제목만 앞 페이지 하단에 남는 고아 제목을 막기 위해, Ⅲ장은 항상 새 페이지에서 시작한다.
    // 앞 페이지에 절반 이상 남을 때만 끼워 넣는 방식은 react-pdf가 사전 잔여 높이를 제공하지
       않아 안정적으로 판별할 수 없으므로, 원본의 장 시작 방식에 맞춘 명시적 페이지 전환을 쓴다. */
    <View break>
      <SectionHeader numeral="III" title="기능별 고객 경험 평가" />
      {/* 원본에는 "기능별 만족도" 일반 소제목이 없다. 장 아래에는 번호 상자 형식의
          "1 | 기능별 고객 경험 조사 결과"가 와야 한다. */}
      <SubsectionHeader number={1} title="기능별 고객 경험 조사 결과" />
      <VerticalBarChartWithAverage
      ...
      {/* 원본 27쪽의 Q12 블록. 이전에는 IV 핵심구매요소(Q13) 뒤에 잘못 붙어 있어
          섹션 계층이 섞였으므로, 만족도 종합표 바로 다음에 둔다. 강제 줄바꿈은 넣지 않아
          단어 단위로만 자연스럽게 개행된다. */}
      <View style={{ flexDirection: "row", marginTop: 12, marginBottom: 6 }}>
        <Text style={[styles.qHeading, { width: 31, marginTop: 0, marginBottom: 0 }]}>Q12.</Text>
        <Text style={[styles.qHeading, { flex: 1, marginLeft: 3, marginTop: 0, marginBottom: 0 }]}>캣독런의 기능 중 중요하다고 생각되는 순위를 1위부터 6위까지 순서대로 작성해주세요.</Text>
      </View>
      {/* 원본처럼 질문과 누적 막대 사이에 시안 구분선을 둬서, 제목·범례·차트가 붙어 보이지
          않도록 한다. */}
      <View style={{ borderTopWidth: 3, borderTopColor: colors.qUnderline, marginTop: 5, marginBottom: 7 }} />
      <RankCompositionChart
        compositions={stats.rankPositionComposition}
        candidateNames={stats.featureSatisfaction.map((f) => f.name)}
      />
      <View wrap={false}>
        <LavenderTitleBand title="기능별 중요 순위 종합" />
        <Text style={[styles.small, { marginTop: 1, marginBottom: 4 }]}> 
          • 상대 중요도: 중요 순위 응답을 0점 기준 +5~-5점 점수로 환산한 값
        </Text>
        <TransposedRankTable
          rowLabel="기능"
          valueLabel="상대 중요도"
          items={[...stats.relativeImportance]
...
function NpsSummaryTable({ nps }: { nps: NpsResult }) {
  ...
  return (
    <View style={[styles.table, { marginTop: 9, borderColor: "#52525b" }]}>
      <View style={{ flexDirection: "row", borderBottomWidth: 1, borderBottomColor: "#52525b" }}>
        {cells.map((cell, i) => (
          <View
            key={cell.label}
            style={{
          ...
          >
            <Text style={{ fontSize: 9.2, lineHeight: 1.35, fontWeight: "bold", textAlign: "center" }}>{cell.label}</Text>
          </View>
        ))}
      </View>
      <View style={{ flexDirection: "row" }}>
        {cells.map((cell, i) => (
          <View key={cell.label} style={{ flex: 1, justifyContent: "center", paddingVertical: 7, paddingHorizontal: 4, ...rightBorder(i) }}>
            <Text style={{ fontSize: 11.2, textAlign: "center" }}>{cell.value}</Text>
          </View>
        ))}
      </View>
    </View>
...
/** 원본 NPS 페이지의 연보라색 실선. 일반 본문 구분선보다 조금 선명하게 유지한다. */
function NpsDivider({ marginTop = 7, marginBottom = 7 }: { marginTop?: number; marginBottom?: number }) {
  return <View style={{ borderTopWidth: 1.15, borderTopColor: "#7391d5", marginTop, marginBottom }} />;
}
...
export function SectionNpsAndImprovement({
  ...
  return (
    <View break>
      ...
      <SubsectionHeader number={1} title="종합 만족도 및 NPS 지수" />
      <View
        style={{
        ...
        wrap={false}
      >
        <Text style={{ fontSize: 11.2, lineHeight: 1.3, color: "#7186ac" }}>
      ...
      <NpsDivider marginTop={3} marginBottom={6} />
      ...
      <View style={{ paddingHorizontal: 5 }}>
        ...
        <Text style={[styles.body, { marginLeft: 9 }]}>자발적으로 구매/추천할 만큼 만족도가 높은 잠재 고객</Text>
        <Text style={[styles.body, { fontWeight: "bold", marginTop: 5 }]}>- 중립 고객 (PASSIVE, 7~8점)</Text>
        <Text style={[styles.body, { marginLeft: 9 }]}>제품에 대한 보증까지는 응하지 않을 가능성이 큰 고객</Text>
        <Text style={[styles.body, { fontWeight: "bold", marginTop: 5 }]}> 
          - 비구매/비추천 고객 (DETRACTOR, 0~6점)
        </Text>
        <Text style={[styles.body, { marginLeft: 9 }]}>실제 비구매 및 다른 이에게도 쓰지 않도록 적극적으로 의견을 표출할 수 있는 고객</Text>
      </View>

      <NpsDivider marginTop={8} marginBottom={1} />
...
function ResultSummaryTable({
  ...
  return (
    // 표 전체의 외곽선을 하나의 View로 두면 페이지가 바뀌는 곳에 빈 테두리 영역이 생긴다.
    // 원본처럼 각 결과 항목을 완결된 행으로 유지해, 남은 공간에 들어가지 않으면 행 전체가
    // 다음 페이지에서 시작하게 한다.
    <View>
      {/* 헤더 행 */}
      <View style={{ flexDirection: "row", borderWidth: 1, borderColor: colors.border }} wrap={false}>
        <Text style={{ width: 92, backgroundColor: colors.chartBannerBg, fontSize: 8.5, fontWeight: "bold", textAlign: "center", padding: 5, borderRightWidth: 1, borderRightColor: colors.border, color: colors.text }}>
          항목
        </Text>
        <Text style={{ flex: 1, backgroundColor: colors.chartBannerBg, fontSize: 8.5, fontWeight: "bold", textAlign: "center", padding: 5, color: colors.text }}>
          주요 의견
        </Text>
      </View>
      ...
      {/* 나머지 항목도 페이지 경계에서 절대 쪼개지지 않는 한 행으로 둔다. 빈 표 조각보다
          다음 페이지의 완결된 행이 원본의 발행 양식과 더 일치한다. */}
      {RESULT_SUMMARY_ITEMS.map((item, i) => (
        <View
          ...
          style={{
            ...
            borderLeftWidth: 1,
            borderRightWidth: 1,
            // 페이지 맨 위에서 시작하는 행에도 상단 테두리가 생긴다. 같은 페이지의
            // 이웃 행에서는 다음 행의 상단선만 사용해 이중선이 되지 않는다.
            borderTopWidth: 1,
            borderBottomWidth: i === RESULT_SUMMARY_ITEMS.length - 1 ? 1 : 0,
            borderColor: colors.border,
          }}
        >
