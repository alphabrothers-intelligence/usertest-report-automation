  ...
  return (
    ...
    >
      <View style={{ backgroundColor: colors.chartBannerBg, paddingVertical: 4 }}>
        <Text style={{ fontSize: 9.5, fontWeight: "bold", textAlign: "center", color: colors.text }}>
          {title}
        </Text>
      </View>
        ...
        {axes.flatMap((a, i) => {
          ...
          return lines.map((line, li) => (
            <Text
              ...
              style={{ fontSize: RADAR_AXIS_LABEL_FONT_SIZE, fontFamily: "Pretendard Report" }}
              textAnchor={anchor}
            >
              {line}
            </Text>
        ...
        {Array.from({ length: gridLevels }, (_, level) => {
          ...
          return (
            <Text
              ...
              x={p.x}
              y={p.y - 1.5}
              style={{ fontSize: 4.5, fontFamily: "Pretendard Report" }}
              textAnchor="middle"
            >
              {value.toFixed(2)}
            </Text>
    ...
    <View style={{ marginBottom: 10 }} wrap={false}>
      {title && (
        <View style={{ backgroundColor: colors.chartBannerBg, paddingVertical: 4, marginBottom: 6 }}>
          <Text style={{ fontSize: 9.5, fontWeight: "bold", textAlign: "center", color: colors.text }}>{title}</Text>
        </View>
      )}
...
// 원본 리바랩스 27페이지의 기능 중요도 순위 구성 도표 색감. 채도가 높은 기본색 대신
// 산호·청록·하늘·세이지·크림·라일락의 파스텔 계열을 사용한다. 데이터 열 순서가 바뀌어도
// 같은 기능은 같은 색을 유지하도록 이름 기반으로 먼저 매핑한다.
const COMPOSITION_PALETTE = ["#FF7373", "#56C7C2", "#52B3CD", "#9BCFB9", "#FFE59A", "#D296D8", "#F6B68C", "#BAC3CF"];

function compositionColorFor(name: string, fallbackIndex: number) {
  if (name.includes("산책")) return "#FF7373";
  if (name.includes("실시간") || name.includes("거점")) return "#56C7C2";
  if (name.includes("성장")) return "#52B3CD";
  if (name.includes("꾸미기")) return "#9BCFB9";
  if (name.includes("레이싱")) return "#FFE59A";
  if (name.includes("교배")) return "#D296D8";
  return COMPOSITION_PALETTE[fallbackIndex % COMPOSITION_PALETTE.length];
}
...
export function RankCompositionChart({
  ...
  const colorFor = (name: string) => compositionColorFor(name, candidateNames.indexOf(name));
  return (
      ...
      <Svg width={width} height={height}>
        ...
        {yTicks.map((v) => (
          <Text
            ...
            x={MARGIN_LEFT - 3}
            y={yScale(v) + 2.5}
            style={{ fontSize: 5.5, fontFamily: "Pretendard Report" }}
            textAnchor="end"
          >
            {v}
          </Text>
          ...
          return items.flatMap((it) => {
            ...
            return lines.map((line, li) => (
              <Text
                ...
                x={px}
                y={py + offsets[li]}
                style={{ fontSize: 6, fontWeight: "bold", fontFamily: "Pretendard Report" }}
                textAnchor="middle"
              >
                {line}
              </Text>
