...
// ── "영역별 참고 지표" 9칸 우선순위 예시 다이어그램 ─────────────────────────────
// 사분면과 같은 밴드(중요도 -2/+2, 만족도 6/8 경계)로 9개 영역을 나누고, 각 영역에 우선순위
// 판정 문구를 넣는다. 예전엔 저해상도 JPEG(319×318)를 그대로 삽입해 확대 시 글자가 깨졌는데
// (2026-07-23 "사진이 너무 깨진다"), 캔버스로 직접 그리면 어떤 크기에서도 선명하다(사분면과
// 완전히 같은 색·경계 공식을 재사용하므로 본 그래프와 항상 일관된다).
const PRIORITY_REF_TEXTS: string[][] = [
  // Top(만족도 8~10): 왼쪽→오른쪽
  ["개선 필요성 적음\n우선순위 최하", "개선 필요성 낮음\n우선순위 하\n추후 고도화", "개선 필요성 낮음\n우선순위 중\n추후 고도화"],
  // Mid(만족도 6~8)
  ["개선 권장\n우선순위 하\n추후 고도화", "개선 필요\n우선순위 중\n추후 고도화", "중요 개선\n우선순위 상\n고도화 필요"],
...
export function renderPriorityReferenceDiagram(width = 300, height = 300): CanvasChart {
  const { ctx, finish } = makeCanvas(width, height);
  const marginLeft = 22;
  const marginTop = 10;
  ...
  colBands.forEach((col) => {
    rowBands.forEach((row) => {
      const score = col.score + row.score - 2;
      const level = PRIORITY_LEVELS.find((p) => p.score === score)!;
      ctx.fillStyle = level.color;
      ctx.fillRect(xScale(col.x0), yScale(row.y1), xScale(col.x1) - xScale(col.x0), yScale(row.y0) - yScale(row.y1));
    });
  });

  // 영역 문구(굵은 검정, 각 영역 중앙 정렬)
  ctx.fillStyle = colors.text;
  ctx.textAlign = "center";
  const colCenters = colBands.map((c) => xScale((c.x0 + c.x1) / 2));
  const rowCenters = rowBands.map((r) => yScale((r.y0 + r.y1) / 2));
  const fontSize = Math.max(6, Math.min(8.5, width / 40));
  const lineH = fontSize + 2;
  ctx.font = `bold ${fontSize}px NotoSansKR`;
  rowBands.forEach((_, ri) => {
    colBands.forEach((__, ci) => {
      const lines = PRIORITY_REF_TEXTS[ri][ci].split("\n");
      const startY = rowCenters[ri] - ((lines.length - 1) * lineH) / 2 + fontSize / 2 - 1;
      lines.forEach((line, li) => ctx.fillText(line, colCenters[ci], startY + li * lineH));
    });
  });

  // 영역 경계(굵은 검정선) — 중요도 -2/+2, 만족도 6/8 + 바깥 테두리
  ctx.strokeStyle = colors.text;
  ctx.lineWidth = 1.4;
  [-2, 2].forEach((v) => {
    ctx.beginPath();
  ...
  // 축 눈금 라벨
  ctx.fillStyle = colors.subtext;
  ctx.font = "6px NotoSansKR";
  for (let v = xMin; v <= xMax; v++) {
    ctx.textAlign = "center";
    ctx.fillText(String(v), xScale(v), height - marginBottom + 9);
  }
  for (let v = yMin; v <= yMax; v++) {
    ctx.textAlign = "right";
    ctx.fillText(String(v), marginLeft - 4, yScale(v) + 2);
  }
  drawVerticalText(ctx, "만족도", marginLeft - 15, marginTop + plotHeight / 2, 6.5, colors.subtext, true);
  ctx.textAlign = "center";
  ctx.font = "bold 6.5px NotoSansKR";
  ctx.fillStyle = colors.subtext;
  ctx.fillText("상대 중요도", marginLeft + plotWidth / 2, height - 3);
