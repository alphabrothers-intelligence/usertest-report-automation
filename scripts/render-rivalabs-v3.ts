...
import { ReportDocument } from "../lib/pdf-rivalabs-v3/ReportDocument";
...
async function main() {
  ...
  const buffer = await renderToBuffer(
    ReportDocument({
    fileName: "리바랩스_원본재현_v3_격리렌더러.pdf",
  ...
  const outputPath = path.join(outputDir, "리바랩스_원본재현_v3_격리렌더러.pdf");
