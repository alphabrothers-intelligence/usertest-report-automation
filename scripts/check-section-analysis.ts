 ...
 *
 * SW형은 Claude 호출 4회(동시 최대 2), 실제품형은 1회다. 원문 응답과 생성 본문은
 * tmp 아티팩트에 기록하지 않는다.
 */
...
async function main() {
  ...
  const requestedSections = productType === "sw"
    ? ["featureExperience", "corePurchaseFactor", "fourValues", "uxQuality"]
    : ["corePurchaseFactor"];
  ...
  try {
    const analyses = await runSectionAnalysesForReport(reportId, { concurrency: 2 });
